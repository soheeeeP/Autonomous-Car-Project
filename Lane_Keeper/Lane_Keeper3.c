...
void cal_lane_position(float *L_point, int line_no, float *lane_pos){
	...
	int *label = new int[line_no];
	memset(label, -1, sizeof(int)*line_no);
	
	int label_no = 0;
	int no_cnt = 0;		//idx for counting edges within the line width
	
	label[0] = label_no + 1;
	
	for(i=0;i<line_no-1;i++){
		
		for(j=i,no_cnt=0;j<line_no;j++){
			if(label[j]==-1){
				distance[i] = fabs(L_point[i] - L_point[j]);
				if((distance[i] >= LINE_WIDTH-LINE_WIDTH_TOL)&&(distance[i] <= LINE_WIDTH + LINE_WIDTH_TOL)){
					label[j] = label[i] = label_no+1;
					
				}
				else{
					//count edges outside the line boundry
					no_cnt+=1;
					printf("no_cnt: %2d\n",no_cnt);
				}
				printf("error[%1d %1d] = %3.1lf\n",i,j,distance[i]);
			}
		}
		if(no_cnt>1) {
				label_no +=1;
				printf("label_no increased %3d\n",label_no);
		}
...
int main(void){
	...
	*/
	mat_image_org_color = imread("/home/pi/SOHEE/AutoCar/C++/Lane_Keeper/3/images/line_2_0.jpg"); 
