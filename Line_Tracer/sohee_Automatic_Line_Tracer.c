...
Mat Draw_Guide_Line(Mat img)
{
	...
	line(result_img, Point(line_center, ASSIST_BASE_LINE-guide_height1*1.2) ,Point(line_center, ASSIST_BASE_LINE+guide_height1*1.2), Scalar(0,0,255), 2,0);
...
int main(void)
{
	...
	while(1)
	{
		...
		vector<Vec4i> linesP;
		HoughLinesP(mat_image_roi_canny_edge, linesP, 1, CV_PI/180,20,10,8);
		...
		for(i=0; i < linesP.size() ;i++)
		{
			...
			intersect_base[i] = intersect[i] + gradient[i]*ASSIST_BASE_LINE; // x = a y + b
			c_x_sum = c_x_sum + (int)intersect_base[i];
			//printf("c_x_sum[%d] = %d %d\n",i,c_x_sum,(int)intersect_base[i] );
